...
def classify_posture_metrics_seated(metrics):
    ...
    TRUNK_LOWER_BOUND = float(os.getenv("TRUNK_LOWER_BOUND", "0.30"))
    TRUNK_UPPER_BOUND = float(os.getenv("TRUNK_UPPER_BOUND", "0.30"))
    ...
    # --- Trunk Posture (Back Curvature) with Upper and Lower Bounds ---
    back_data = metrics.get("backCurvature", {})
    back_value = back_data.get("value", 0)
    back_conf = back_data.get("confidence", 0)
    # Define lower and upper bounds via environment variables (with defaults)
    TRUNK_LOWER_BOUND = float(os.getenv("TRUNK_LOWER_BOUND", "0.05"))
    TRUNK_UPPER_BOUND = float(os.getenv("TRUNK_UPPER_BOUND", "0.35"))
    if back_conf < 0.5:
        classification["trunk"] = "unknown"
    else:
        # Check if the back curvature is within the absolute bounds
        if not (TRUNK_LOWER_BOUND <= back_value <= TRUNK_UPPER_BOUND):
            classification["trunk"] = "acceptable"
        else:
            if TRUNK_LOWER_BOUND <= back_value+TRUNK_ACCEPTABLE_THRESHOLD <= TRUNK_UPPER_BOUND:
                classification["trunk"] = "not recommended"
            elif TRUNK_LOWER_BOUND > back_value-TRUNK_WARNING_THRESHOLD or back_value-TRUNK_WARNING_THRESHOLD > TRUNK_UPPER_BOUND:
...
def posture_detection(stream=True, debug_view=True):
    ...
    def update_posture_data():
        global smoothed_curvature, brk
        display_scale = 1
        url = "http://192.168.79.74:8080/photo.jpg"
        ...
        while not brk:
            ...
            # Resize and convert color space as needed
            frame = cv2.resize(frame, (640, 480))
            frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

            # Process the image through your pose estimator
            results = pose.process(frame_rgb)

            # Calculate curvature and other metrics
            ...
            # Update posture_data with a lock
            with posture_data_lock:
                posture_data.update(
                    {
                        "trust": trust,
            ...
            # Create and queue the message update
